...
import { getMerchant, getMerchantsList } from "@/lib/api";
...
const ExportButton = dynamic(
  () =>
    import("@/components/export/ExportButton").then((mod) => ({
...
export default function MerchantPortfolioPage() {
  ...
  const [prefetchingMerchantId, setPrefetchingMerchantId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (currentPath && currentPath.startsWith("/merchant-details/")) {
      ...
      const merchantId = currentPath
        .replace("/merchant-details/", "")
        .split("/")[0]
        .split("?")[0];
              ...
              fallback={
                <div className="container mx-auto p-6">
                  <Alert variant="destructive">
                    <AlertDescription>
                      Failed to load merchant details page. Please try
                      refreshing the page.
                    </AlertDescription>
                  </Alert>
                </div>
  ...
  const fetchMerchants = useCallback(async () => {
    ...
    try {
      ...
      const verified = response.merchants.filter(
        (m) => m.status === "active" || m.compliance_status === "compliant"
      ).length;
      const pending = response.merchants.filter(
        (m) => m.status === "pending"
      ).length;
      const highRisk = response.merchants.filter(
  ...
  useEffect(() => {
    ...
    merchants.forEach((merchant) => {
      if (merchant.updated_at) {
        try {
          formatted[merchant.id] = new Date(
            merchant.updated_at
  ...
  const handleMerchantHover = useCallback(
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Search and filter your merchant portfolio
                  </CardDescription>
                </div>
                    ...
                    {loading ? (
                        ...
                        >
                          ...
                          riskLevelFilter !== "all" ? (
                            ...
                            <Button
                              ...
                              className="mt-4"
                              variant="outline"
                              aria-label="Add your first merchant"
                            >
                              <Link href="/add-merchant">
                      ...
                      merchants.map((merchant) => (
                        ...
                        >
                          ...
                          <TableCell>
                            <Badge
                              variant={getRiskBadgeVariant(merchant.risk_level)}
                            >
                              {merchant.risk_level || "N/A"}
                            </Badge>
                          </TableCell>
                          <TableCell suppressHydrationWarning>
                            {mounted && formattedDates[merchant.id]
                              ? formattedDates[merchant.id]
                              : merchant.updated_at || "N/A"}
                          </TableCell>
