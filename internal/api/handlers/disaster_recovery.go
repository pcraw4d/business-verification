...
func (h *DisasterRecoveryHandler) GetStatus(w http.ResponseWriter, r *http.Request) {
	h.logger.Info("Getting disaster recovery status")
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		h.logger.Error("Failed to encode DR status response", "error", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}

	h.logger.Info("DR status retrieved successfully")
}
...
func (h *DisasterRecoveryHandler) GetHealthStatus(w http.ResponseWriter, r *http.Request) {
	h.logger.Info("Getting disaster recovery health status")
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		h.logger.Error("Failed to encode DR health status response", "error", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}

	h.logger.Info("DR health status retrieved successfully")
}
...
func (h *DisasterRecoveryHandler) InitiateFailover(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		h.logger.Error("Failed to decode failover request", "error", err)
		http.Error(w, "Invalid request body", http.StatusBadRequest)
		return
	}

	h.logger.Info("Initiating disaster recovery failover", "force", req.Force)
	...
	if err != nil {
		...
		h.logger.Error("Failover failed", "error", err, "duration", duration)
		...
		h.logger.Info("Failover completed successfully", "duration", duration)
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		h.logger.Error("Failed to encode failover response", "error", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}
}
...
func (h *DisasterRecoveryHandler) InitiateFailback(w http.ResponseWriter, r *http.Request) {
	...
	var req FailoverRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		h.logger.Error("Failed to decode failback request", "error", err)
		http.Error(w, "Invalid request body", http.StatusBadRequest)
		return
	}

	h.logger.Info("Initiating disaster recovery failback", "force", req.Force)
	...
	if err != nil {
		response.Error = err.Error()
		h.logger.Error("Failback failed", "error", err, "duration", duration)
		w.WriteHeader(http.StatusInternalServerError)
	} else {
		// Get updated status to determine target region
		updatedStatus := h.drService.GetStatus()
		response.ToRegion = updatedStatus.CurrentRegion
		h.logger.Info("Failback completed successfully", "duration", duration)
		w.WriteHeader(http.StatusOK)
	}
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		h.logger.Error("Failed to encode failback response", "error", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}
}
...
func (h *DisasterRecoveryHandler) TestFailover(w http.ResponseWriter, r *http.Request) {
	h.logger.Info("Testing disaster recovery failover")
	...
	if err != nil {
		response["error"] = err.Error()
		h.logger.Error("Test failover failed", "error", err)
		w.WriteHeader(http.StatusInternalServerError)
	} else {
		h.logger.Info("Test failover completed successfully")
		w.WriteHeader(http.StatusOK)
	}
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		h.logger.Error("Failed to encode test failover response", "error", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}
}
...
func (h *DisasterRecoveryHandler) TestFailback(w http.ResponseWriter, r *http.Request) {
	h.logger.Info("Testing disaster recovery failback")
	...
	if err != nil {
		response["error"] = err.Error()
		h.logger.Error("Test failback failed", "error", err)
		w.WriteHeader(http.StatusInternalServerError)
	} else {
		h.logger.Info("Test failback completed successfully")
		w.WriteHeader(http.StatusOK)
	}
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		h.logger.Error("Failed to encode test failback response", "error", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}
}
...
func (h *DisasterRecoveryHandler) EnableAutoFailover(w http.ResponseWriter, r *http.Request) {
	h.logger.Info("Enabling auto-failover")
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		h.logger.Error("Failed to encode auto-failover enable response", "error", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}

	h.logger.Info("Auto-failover enabled successfully")
}
...
func (h *DisasterRecoveryHandler) DisableAutoFailover(w http.ResponseWriter, r *http.Request) {
	h.logger.Info("Disabling auto-failover")
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		h.logger.Error("Failed to encode auto-failover disable response", "error", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}

	h.logger.Info("Auto-failover disabled successfully")
}
...
func (h *DisasterRecoveryHandler) EnableAutoFailback(w http.ResponseWriter, r *http.Request) {
	h.logger.Info("Enabling auto-failback")
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		h.logger.Error("Failed to encode auto-failback enable response", "error", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}

	h.logger.Info("Auto-failback enabled successfully")
}
...
func (h *DisasterRecoveryHandler) DisableAutoFailback(w http.ResponseWriter, r *http.Request) {
	h.logger.Info("Disabling auto-failback")
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		h.logger.Error("Failed to encode auto-failback disable response", "error", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}

	h.logger.Info("Auto-failback disabled successfully")
}
...
func (h *DisasterRecoveryHandler) GetFailoverHistory(w http.ResponseWriter, r *http.Request) {
	h.logger.Info("Getting failover history")
	...
	if err := json.NewEncoder(w).Encode(history); err != nil {
		h.logger.Error("Failed to encode failover history response", "error", err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}

	h.logger.Info("Failover history retrieved successfully")
}
