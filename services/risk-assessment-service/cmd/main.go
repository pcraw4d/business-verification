...
func main() {
	...
	// Initialize performance components
	perfComponents, err := initPerformanceComponents(cfg, db, logger)
	if err != nil {
		logger.Fatal("Failed to initialize performance components", zap.Error(err))
	}
	defer perfComponents.Close()
	...
	performanceMonitor := monitoring.NewPerformanceMonitor(logger)
	performanceMonitor.SetTargets(16.67, 1*time.Second, 0.01, 1000)
	logger.Info("✅ Performance monitor initialized")
	...
	alertManager.AddChannel("email", emailChannel)

	// Initialize monitoring components
	monitoringComponents := &monitoring.MonitoringComponents{
	...
	// Initialize handlers
	riskAssessmentHandler := handlers.NewRiskAssessmentHandler(riskEngine, externalDataService, logger)
	advancedPredictionHandler := handlers.NewAdvancedPredictionHandler(mlService, logger)
	externalAPIHandler := handlers.NewExternalAPIHandler(externalAPIManager, logger)
	metricsHandler := handlers.NewMetricsHandler(prometheusMetrics, logger)
	monitoringHandler := handlers.NewMonitoringHandler(monitoringComponents, logger)
	performanceHandler := handlers.NewPerformanceHandler(performanceMonitor, logger)
	...
	router.Use(middlewareInstance.RecoveryMiddleware())
	router.Use(middlewareInstance.LoggingMiddleware)
	router.Use(legacyPerformanceMiddleware.Middleware)
	...
	router.Use(middlewareInstance.RateLimitMiddleware(100))
	router.Use(middlewareInstance.MetricsMiddleware())
	router.Use(middlewareInstance.HealthCheckMiddleware())
	...
	api.HandleFunc("/external/verify", externalAPIHandler.HandleExternalVerification).Methods("POST")
	api.HandleFunc("/external/search", externalAPIHandler.HandleExternalSearch).Methods("POST")
	...
	api.HandleFunc("/monitoring/health", monitoringHandler.GetHealth).Methods("GET")
	api.HandleFunc("/monitoring/alerts", monitoringHandler.GetAlerts).Methods("GET")
	api.HandleFunc("/monitoring/dashboard", monitoringHandler.GetDashboard).Methods("GET")
	...
	// External API routes
	externalAPI := router.PathPrefix("/external").Subrouter()
	externalAPI.HandleFunc("/health", externalAPIHandler.HealthCheck).Methods("GET")
