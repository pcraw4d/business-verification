...
# Initialize Supabase Database Schema for Keyword Classification System
# This script sets up the complete database schema and seeds initial data

set -e

echo "🚀 Initializing Supabase Database Schema for Keyword Classification System"
...
TEMP_SQL_FILE="/tmp/keyword_classification_schema.sql"

echo "📝 Creating schema migration file..."
...
CREATE INDEX IF NOT EXISTS idx_code_keywords_relevance ON code_keywords(relevance_score);
...
CREATE INDEX IF NOT EXISTS idx_industry_patterns_priority ON industry_patterns(priority);
...
CREATE INDEX IF NOT EXISTS idx_keyword_weights_calculated ON keyword_weights(calculated_weight);
...
DROP POLICY IF EXISTS "Allow public read access to industries" ON industries;
CREATE POLICY "Allow public read access to industries" ON industries
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Allow public read access to industry_keywords" ON industry_keywords;
CREATE POLICY "Allow public read access to industry_keywords" ON industry_keywords
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Allow public read access to classification_codes" ON classification_codes;
CREATE POLICY "Allow public read access to classification_codes" ON classification_codes
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Allow public read access to code_keywords" ON code_keywords;
CREATE POLICY "Allow public read access to code_keywords" ON code_keywords
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Allow public read access to industry_patterns" ON industry_patterns;
CREATE POLICY "Allow public read access to industry_patterns" ON industry_patterns
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Allow public read access to keyword_weights" ON keyword_weights;
...
DROP POLICY IF EXISTS "Allow authenticated users to manage industries" ON industries;
CREATE POLICY "Allow authenticated users to manage industries" ON industries
    FOR ALL USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow authenticated users to manage industry_keywords" ON industry_keywords;
CREATE POLICY "Allow authenticated users to manage industry_keywords" ON industry_keywords
    FOR ALL USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow authenticated users to manage classification_codes" ON classification_codes;
CREATE POLICY "Allow authenticated users to manage classification_codes" ON classification_codes
    FOR ALL USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow authenticated users to manage code_keywords" ON code_keywords;
CREATE POLICY "Allow authenticated users to manage code_keywords" ON code_keywords
    FOR ALL USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow authenticated users to manage industry_patterns" ON industry_patterns;
CREATE POLICY "Allow authenticated users to manage industry_patterns" ON industry_patterns
    FOR ALL USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow authenticated users to manage keyword_weights" ON keyword_weights;
CREATE POLICY "Allow authenticated users to manage keyword_weights" ON keyword_weights
    FOR ALL USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow authenticated users to manage audit_logs" ON audit_logs;
...
echo "✅ Schema migration file created: $TEMP_SQL_FILE"

# Execute the schema migration using curl to Supabase REST API
...
RESPONSE=$(curl -s -X POST \
  ...
  -d "{\"sql\": \"$(cat $TEMP_SQL_FILE | sed 's/"/\\"/g' | tr '\n' ' ')\"}")
...
echo "🎉 Supabase database schema initialization completed successfully!"
echo "📋 Tables created:"
echo "   - industries"
...
echo "🔄 Triggers for updated_at timestamps created"
echo "📚 Documentation comments added"
echo ""
echo "✅ Database is ready for the new keyword classification system!"
