...
# This script runs comprehensive foreign key constraint tests
...
run_sql_tests() {
    local db_url=$1
    local output_file="$OUTPUT_DIR/foreign_key_sql_test_$TIMESTAMP.log"
    
    print_status $BLUE "📊 Running SQL-based foreign key tests..."
    
    if command_exists psql; then
        print_status $BLUE "   Executing SQL test script..."
        if psql "$db_url" -f "$SCRIPT_DIR/test-foreign-keys.sql" > "$output_file" 2>&1; then
            ...
            if grep -q "orphaned_count.*[1-9]" "$output_file"; then
...
run_go_tests() {
    local db_url=$1
    local output_file="$OUTPUT_DIR/foreign_key_go_test_$TIMESTAMP.log"
    
    print_status $BLUE "🔧 Running Go-based foreign key tests..."
    ...
    if command_exists go; then
        ...
        if go build -o test-foreign-keys test-foreign-keys.go; then
            print_status $GREEN "✅ Go program compiled successfully"
            
            # Run the test
            if ./test-foreign-keys "$db_url" > "$output_file" 2>&1; then
...
generate_summary_report() {
    local output_file="$OUTPUT_DIR/foreign_key_test_summary_$TIMESTAMP.md"
...
EOF
    ...
    if [ -f "$OUTPUT_DIR/foreign_key_sql_test_$TIMESTAMP.log" ]; then
        echo "✅ SQL tests completed" >> "$output_file"
        echo "📁 Results: \`foreign_key_sql_test_$TIMESTAMP.log\`" >> "$output_file"
    else
...
EOF
    ...
    if [ -f "$OUTPUT_DIR/foreign_key_go_test_$TIMESTAMP.log" ]; then
        echo "✅ Go tests completed" >> "$output_file"
        echo "📁 Results: \`foreign_key_go_test_$TIMESTAMP.log\`" >> "$output_file"
    else
...
2. Address any orphaned records found
3. Fix any constraint violations
4. Re-run tests to verify fixes

## Files Generated

- \`foreign_key_sql_test_$TIMESTAMP.log\` - SQL test results
- \`foreign_key_go_test_$TIMESTAMP.log\` - Go test results
- \`foreign_key_test_summary_$TIMESTAMP.md\` - This summary report
...
# Main execution
main() {
    print_status $BLUE "🚀 Starting Foreign Key Constraint Testing"
    print_status $BLUE "=========================================="
    ...
    if [ $sql_result -eq 0 ] && [ $go_result -eq 0 ]; then
        print_status $GREEN "🎉 All foreign key constraint tests completed successfully!"
